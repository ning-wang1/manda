...
CONFIDENCE = 0.005  # how strong the adversarial example should be
INITIAL_CONST = 1e-3  # the initial constant c to pick as a first guess

modified_fea = list(range(1, 30)) + list(range(34, 43)) + list(range(51, 56)) + list(range(62, 66))
mask = [1 if i in modified_fea else 0 for i in range(81)]


class CarliniL2_res_2d:
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
        ...
        min_value = [-10 if x in modified_fea else 0 for x in range(model.image_size**2)]
        max_value = [10 if x in modified_fea else 0 for x in range(model.image_size**2)]
        mask = [1.0 if i in modified_fea else 0.0 for i in range(81)]
        self.modified_grad = []
        for grad, var in self.get_grad:
            grad_shape = grad.shape
            grad_new = tf.reshape(grad, (1, 81), name=None)

            mask = tf.constant(mask, shape=[1, 81])
            clipped_grad = tf.multiply(grad_new, mask)
            # clipped_grad = tf.clip_by_value(grad_new, clip_value_min=min_value, clip_value_max=max_value)
            self.modified_grad.append((tf.reshape(clipped_grad, grad_shape), var))
